$(function() {
	...
	oTable1 = $('#table_report').dataTable( {
        ...
        "bServerSide": true, // 从服务器端取数据
       	"sAjaxSource": $('#ctx').val() + "/pages/ftpfolder/form/manager?now=" + new Date().getTime(), //mvc后台ajax调用接口。
       	"fnServerParams": function(aoData) { // 以下是For查询
       		aoData.push({"name" : "ftpName", "value" : $("#query_ftpName").val()});
       		aoData.push({"name" : "folderName", "value" : $("#query_folderName").val()});
       	},
        ...
        "aoColumns": [ { 
    			...
    			"sTitle": "FTP服务器名称",
                "mDataProp": "ftpConfig.ftpName",
			},{
    			"sTitle": "文件夹名称",
                "mDataProp": "folderName",
			},{
    			"sTitle": "配置路径",
                "mDataProp": "url",
			},{
