...
//增加
addObj = function() {
	var url = $('#ctx').val() + "/pages/ftpfolder/form/add";
	var commonModalCss = {
		"width": "450px",
...
$(function() {
	var options = {
		...
		url : $('#ctx').val() + "/pages/ftpfolder/form/addSubmit",
		type : 'post',
		clearForm : false,
		timeout : 3000
	};
	$('#FtpFolderForm').ajaxForm(options);
});
...
//显示修改
modObj = function(id) {
	var url = $('#ctx').val() + "/pages/ftpfolder/form/edit?id=" + id;
	var commonModalCss = {
		"width": "450px",
		"margin": "100px 0 0 -450px"
	};
...
$(function() {
	var options = {
		success : showResponse,
		url : $('#ctx').val() + "/pages/ftpfolder/form/editSubmit",
		type : 'post',
		clearForm : false,
		timeout : 3000
	};
	$("#FtpFolderFormEdit").ajaxForm(options);
});

//删除信息
delObj = function(id) {
	openConfirmModalInFrame("您确定删除该信息吗？",function(){
		var url = $('#ctx').val()+"/pages/ftpfolder/form/delete?id="+id;
		$.ajax( {
			"dataType": 'json',
			"type": "POST",
			"url": url,
