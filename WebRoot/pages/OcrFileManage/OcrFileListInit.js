$(function() {
	...
	oTable1 = $('#table_report').dataTable( {
        ...
        "bServerSide": true, // 从服务器端取数据
       	"sAjaxSource": $('#ctx').val() + "/pages/OcrFileManage/form/manager?now=" + new Date().getTime(), //mvc后台ajax调用接口。
       	"fnServerParams": function(aoData) { // 以下是For查询
       		aoData.push({"name" : "fileName", "value" : $("#query_fileName").val()});
       	},
        ...
        "aoColumns": [ { 
    			...
    			"sTitle": "接收文件路径",
                "mDataProp": "receiveFilePath",
			},{
    			"sTitle": "原始文件路径",
                "mDataProp": "originalFilePath",
			},{
    			"sTitle": "操作",
    			"mData": null,
    	        "aTargets": [ -1 ],
                ...
                "fnRender": function (oObj) {
                    ...
                    var preview = '<button class="btn btn-mini btn-inverse" title="预览原始文件" onclick="previewObj(\'' + oObj.aData.id + '\')"><i class="icon-eye-open bigger-120"></i></button>';
                    var end = '</div>';
                    return start + edit + trash + transform + preview + end;
            	}
			}
        ],
