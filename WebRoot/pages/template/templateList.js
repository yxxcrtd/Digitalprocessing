...
//增加
addObj = function() {
	var url = $('#ctx').val() + "/pages/template/form/edit";
	var commonModalCss = {
		"width": "450px",
...
//显示修改
modObj = function(id) {
	var url = $('#ctx').val() + "/pages/template/form/edit?id=" + id;
	var commonModalCss = {
		"width": "450px",
		"margin": "100px 0 0 -450px"
	};
...
$(function() {
	var options = {
		...
		url : $('#ctx').val() + "/pages/template/form/editSubmit",
		type : 'post',
		clearForm : false,
		timeout : 3000
	};
	$("#templateform").ajaxForm(options);
});

//删除信息
delObj = function(id) {
	openConfirmModalInFrame("您确定删除该信息吗？",function(){
		var url = $('#ctx').val()+"/pages/template/form/delete?id="+id;
		$.ajax( {
			"dataType": 'json',
			"type": "POST",
			"url": url,
