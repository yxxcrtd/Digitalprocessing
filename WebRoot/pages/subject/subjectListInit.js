$(function() {
	...
	oTable1 = $('#table_report').dataTable( {
        ...
        "bServerSide": true, // 从服务器端取数据
       	"sAjaxSource": $('#ctx').val() + "/pages/subject/form/manager?now=" + new Date().getTime(), //mvc后台ajax调用接口。
        ...
        "aoColumns": [ { 
                ...
                "fnRender": function (oObj) {
                    ...
                    var configval = '<button class="btn btn-mini btn-warning" title="配置" onclick="generate(\''+ oObj.aData.id+'\',\''+oObj.aData.treeCode +'\')"><i class="icon-wrench bigger-120"></i></button>';
                    var end = '</div>';
                    return start + edit + trash + configval + end;
            	}
			}
        ],
