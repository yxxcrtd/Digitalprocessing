...
$(function() {
	var options = {
		...
		url : $('#ctx').val() + "/pages/ftpconfig/form/addSubmit",
		type : 'post',
		clearForm : false,
		timeout : 3000
	};
	$('#FtpConfigForm').ajaxForm(options);
});
...
// 显示修改
modObj = function(id) {
	var url = $('#ctx').val() + "/pages/ftpconfig/form/edit?id=" + id;
	var commonModalCss = {
		"width": "450px",
		"margin": "100px 0 0 -450px"
	};
...
$(function() {
	var options = {
		success : showResponse,
		url : $('#ctx').val() + "/pages/ftpconfig/form/editSubmit",
		type : 'post',
		clearForm : false,
		timeout : 3000
	};
	$('#FtpConfigFormEdit').ajaxForm(options);
});
...
delObj = function(id) {
	openConfirmModalInFrame("您确定删除该信息吗？",function(){
		var url = $('#ctx').val()+"/pages/ftpconfig/form/delete?id="+id;
		$.ajax( {
			"dataType": 'json',
			"type": "POST",
			"url": url,
